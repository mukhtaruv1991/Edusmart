...
export default function ParentsList() {
  ...
  useEffect(() => {
    ...
    // Fetch parents linked to this school
    const q = query(
      collection(db, 'users'),
      where('role', '==', 'parent'),
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        fetchedParents.push({ uid: doc.id, ...doc.data() } as Parent);
      });
      setParents(fetchedParents);
      setLoading(false);
    }, (error) => {
      console.error('Error fetching parents:', error);
      toast.error(language === 'en' ? 'Failed to load parents' : 'فشل في تحميل أولياء الأمور');
      setLoading(false);
    });
  ...
  const handleStatusChange = async (parentId: string, newStatus: 'active' | 'inactive') => {
    try {
      await updateDoc(doc(db, 'users', parentId), { status: newStatus });
      toast.success(
        language === 'en' 
          ? `Parent status updated to ${newStatus}` 
          : `تم تحديث حالة ولي الأمر إلى ${newStatus === 'active' ? 'نشط' : 'غير نشط'}`
      );
    } catch (error) {
  ...
  const handleDelete = async (parentId: string) => {
    if (!window.confirm(language === 'en' ? 'Are you sure you want to remove this parent?' : 'هل أنت متأكد أنك تريد إزالة ولي الأمر هذا؟')) {
      return;
    }

    try {
      await deleteDoc(doc(db, 'users', parentId));
      toast.success(language === 'en' ? 'Parent removed successfully' : 'تم إزالة ولي الأمر بنجاح');
    } catch (error) {
      console.error('Error deleting parent:', error);
      toast.error(language === 'en' ? 'Failed to remove parent' : 'فشل في إزالة ولي الأمر');
    }
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
            ...
            {language === 'en' ? 'Parents Management' : 'إدارة أولياء الأمور'}
          </h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            {language === 'en' ? 'Manage parents in your school' : 'إدارة أولياء الأمور في مدرستك'}
          </p>
        </div>
      </div>
        ...
        <div className="p-4 border-b border-gray-100 dark:border-gray-700">
          <div className="relative max-w-md">
            ...
            <input 
              ...
              placeholder={language === 'en' ? 'Search parents...' : 'البحث عن أولياء الأمور...'}
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {loading ? (
                ...
                <tr>
                  <td colSpan={4} className="p-8 text-center text-gray-500 dark:text-gray-400">
                    {language === 'en' ? 'No parents found.' : 'لم يتم العثور على أولياء أمور.'}
                  </td>
                </tr>
              ) : (
                filteredParents.map((parent) => (
                  <tr key={parent.uid} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    <td className="p-4">
                    ...
                    <td className="p-4">
                      <p className="text-sm text-gray-600 dark:text-gray-300">{parent.phone}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">{parent.email}</p>
                    </td>
                    <td className="p-4">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        parent.status === 'active' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' :
                        parent.status === 'pending' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400' :
                        'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400'
                      }`}>
                        {parent.status === 'active' ? (language === 'en' ? 'Active' : 'نشط') :
                         parent.status === 'pending' ? (language === 'en' ? 'Pending' : 'قيد الانتظار') :
                    ...
                    <td className="p-4">
                      <div className="flex items-center justify-center gap-2">
                        {parent.status !== 'active' && (
                          <button 
                            onClick={() => handleStatusChange(parent.uid, 'active')}
                        ...
                        {parent.status === 'active' && (
                          <button 
                            onClick={() => handleStatusChange(parent.uid, 'inactive')}
