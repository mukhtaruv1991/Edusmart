...
## مسار التشغيل دون Firebase Storage

عند عدم توفر Firebase Storage بسبب عدم تفعيل الفوترة، يستخدم التطبيق قاعدة Firestore ذات المعرّف `default` لفهرسة الكتب في `curriculumBooks` وحفظ النص المعالج في مجموعة فرعية باسم `contentChunks`. يحتوي سجل الكتاب على `sourceUrl` و`pdfUrl` كرابطين للمصدر الرسمي، إضافة إلى `contentMode: firestore_chunks` و`contentChunkCount` للتحقق من اكتمال النص.

هذا المسار لا ينسخ ملفات PDF إلى Firestore؛ فالـPDF يبقى في رابط وزارة التربية الرسمي، بينما تُحفظ الصفحات النصية فقط في chunks لا تتجاوز الحجم الآمن للوثيقة. يسمح ذلك بالبحث والشرح وإنشاء الأسئلة من النص، لكنه لا يوفر تنزيل PDF دون اتصال ولا يضمن استقرار المصدر الخارجي إذا تغيّر الرابط.

يجب نشر قواعد `firestore.rules` المعدلة حتى يتمكن المستخدم المصادق عليه من قراءة `contentChunks` للكتاب النشط والمعتمد. إذا لم تكن صلاحية نشر القواعد متاحة لحساب Service Account، ينشر مالك المشروع القواعد من Firebase Console أو يمنح الحساب صلاحية إدارة قواعد Firestore؛ لا يحتاج هذا الإجراء إلى إنشاء قاعدة بيانات جديدة.
